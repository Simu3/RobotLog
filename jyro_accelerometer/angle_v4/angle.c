...
void main()
{
  ...
  while(1){
    ...
    if((-1.0 <= jyro_x) && (jyro_x <= 1.0) && (-0.5 <= new_angle_x) && (new_angle_x <= 0.5))
      ...
      new_angle_x = old_angle_x - (W * old_angle_x * TIME) + (W * angle_x * TIME) + (W * jyro_x * TIME);
    if((-1.0 <= jyro_y) && (jyro_y <= 1.0) && (-0.5 <= new_angle_y) && (new_angle_y <= 0.5))
      new_angle_y = 0.0;
    else
      new_angle_y = old_angle_y - (W * old_angle_y * TIME) + (W * angle_y * TIME) + (W * jyro_y * TIME);
