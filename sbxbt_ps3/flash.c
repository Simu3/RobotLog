// PIC Internal Flash ACCESS function
//
...
void save_local_bluetooth_address(BYTE address[6]){
	if(Check_saved_local_bluetooth_address(address) == FALSE){//eeprom saved?
		//not saved,new save
		erase_flashmem(SECTOR_START);
		prog_flashmem_row2(SECTOR_START,address,6,NULL,NULL);
		log_info("save_local_bluetooth_address:saved\n");
	}else{
		log_info("save_local_bluetooth_address:already saved\n");
	}
}
//BOOL
// FALSE:EEPROM not saved
// TRUE:EEPROM saved
BOOL read_local_bluetooth_address(BYTE address[6]){
	int i;
	int erasedFlag = 1;
	//read eeprom
	read_flashmem(SECTOR_START,address,6);
	log_info("read_local_bluetooth_address:read %02x %02x %02x %02x %02x %02x\n",
		address[0],
		address[1],
		address[2],
		address[3],
...
//BOOL
// FALSE:Not saved
// TRUE:That address already saved
BOOL Check_saved_local_bluetooth_address(BYTE address[6]){
	BYTE savedAddress[6];
	int savedFlag = TRUE;
	int i;
	if(read_local_bluetooth_address(savedAddress)){//EEPROM saved?
		for(i = 0;i < 6;i++){
			if(address[i] != savedAddress[i]){
				savedFlag = FALSE;
				break;
			}
		}
