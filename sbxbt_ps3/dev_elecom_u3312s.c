...
static void elecom_u3312s_process_packet(unsigned char *hid_report,uint16_t size)
{
	WORD	button_state;
	//digital state
	button_state=0;
	...
	sixaxis_lx=hid_report[0];
	sixaxis_ly=hid_report[1];
		...
		switch(state){
			...
			case CONTROLLER_STATE_GET_DATA_WAIT:
				if (!USBHostBluetoothRxEventIsBusy(deviceAddress)){
					...
					log_info( "GET DATA\r\n" );
#endif
					elecom_u3312s_process_packet(hid_packet,64);
					out_button_state();
				}
				break;
